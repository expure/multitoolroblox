...
local containers = {}
local function addContainer(name, ypos)
    local c = Instance.new("Frame")
    c.Size = UDim2.new(0.9, 0, 0.06, 0)
...
instructionLabel.Size = UDim2.new(0.96, 0, 0.28, 0)
instructionLabel.Position = UDim2.new(0.02, 0, 0.02, 0)
instructionLabel.BackgroundTransparency = 1
instructionLabel.Text = "The recommended speed-up method is Legit. Use the Strength mode to push and pull heavy objects! If Grabber or Puncher aren’t working, try nudging, jumping, or touching the target object until the functions activate. Y-Stab prevents you from flying upward while running quickly, and Move-Stab stops your Character from moving without your knowledge. To use AimBot, hold down the RMB and the camera will snap to the nearest player. The Fused tab contains a variety of scripts and functions that may be useful, but they might be non-functional or low quality, so use them with caution."
...
local function updateStrength()
    ...
    if strengthBodyVelocity then
        ...
        if hrp then
            ...
            hrp.Mass = math.clamp(hrp.Mass * currentSpeedMult, 1000, 1e8)
        end
    end
end
...
local function updateTextSizes()
    ...
    if instructionLabel and instructionLabel.Parent then instructionLabel.TextSize = math.floor(13 * scale) end
end
    ...
    conn = RunService.Heartbeat:Connect(function()
        if resizing then
            ...
            frame.Size = UDim2.new(
                ...
                math.max(0.5, startSize.Y.Scale + delta.Y / gui.AbsoluteSize.Y), 0
            )
        else
            conn:Disconnect()
        end
